...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.unidades.update', $unidade->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('responsavel') border-red-500 @enderror" 
                       ...
                       type="text" 
                       name="responsavel" 
                       value="{{ old('responsavel', $unidade->responsavel) }}">
                @error('responsavel')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('estado') border-red-500 @enderror" 
                           ...
                           type="text" 
                           name="estado" 
                           value="{{ old('estado', $unidade->estado) }}"
                           maxlength="2"
                           placeholder="UF">
                    @error('estado')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('observacoes') border-red-500 @enderror" 
                          id="observacoes" 
                          name="observacoes" 
                          rows="3">{{ old('observacoes', $unidade->observacoes) }}</textarea>
                @error('observacoes')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" 
                           name="ativo" 
                           value="1" 
                           {{ old('ativo', $unidade->ativo) ? 'checked' : '' }}
                           class="mr-2">
